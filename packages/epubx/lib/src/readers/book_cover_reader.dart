...
class BookCoverReader {
  static Future<images.Image?> readBookCover(EpubBookRef bookRef) async {
    ...
    var coverManifestItem = bookRef.Schema!.Package!.Manifest!.Items!.firstWhereOrNull((EpubManifestItem manifestItem) => manifestItem.Id!.toLowerCase() == coverMetaItem.Content!.toLowerCase());
    ...
    if (coverImageContentFileRef == null) {
      final coverFile = bookRef.Content!.AllFiles![coverManifestItem.Href];
      if (coverFile == null) return null;
      var coverImageContent = await coverFile!.readContentAsBytes();
