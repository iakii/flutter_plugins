...
class EpubReader {
  ...
  static Future<EpubBookRef> openBook(FutureOr<List<int>> bytes) async {
    ...
    bookRef.Title = bookRef.Schema!.Package!.Metadata!.Titles!.firstWhere((String name) => true, orElse: () => '');
    bookRef.AuthorList = bookRef.Schema!.Package!.Metadata!.Creators!.map((EpubMetadataCreator creator) => creator.Creator).toList();
